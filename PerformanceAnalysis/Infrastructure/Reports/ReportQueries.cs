...
{
    // Хранилище SQL-запросов для формирования аналитических отчетов
    public static class ReportQueries
    {
        // Рейтинг студентов по общей сумме баллов
        ...
        // Анализ тренда активности группы по месяцам
        ...
        // Распределение активности студентов по часам суток
        ...
        // Статистика лидеров и отстающих в группах
        public const string GroupLeadersAndLaggards = @"
            WITH StudentScores as (
                ...
                WHERE (@DirectionId IS NULL OR g.directionid = @DirectionId) AND (@CourseId IS NULL OR g.courseid = @CourseId)
        ...
        // Процент прохождения тестов по студентам
        ...
        // Сводка по успеваемости конкретного студента
        ...
        // Анализ прохождения тестов по дням недели
        ...
        // Детальные результаты тестов студента
